...
class EventDispatchTask(private val event: GuildMessageReceivedEvent, private var message: String, private var prefix: String) : Runnable {
    override fun run() {
        ...
        for(command in getCommands()) {
            ...
            if(message.startsWith(command.name.lowercase()) && event.member?.hasPermission(command.requiredPermission) == true) {
                command.run(event.message, message.extractArguments(prefix, command.name), prefix); return
            }
            ...
            for(alias in command.aliases) {
                ...
                if(message.startsWith(alias.lowercase()) && event.member?.hasPermission(command.requiredPermission) == true) {
                    command.run(event.message, message.extractArguments(prefix, command.name), prefix)
                    return
                }
            }
        }
